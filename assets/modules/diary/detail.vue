<template lang="jade">
    div.wuji-container.center-block(v-if="diary")
        ...
        div.wuji-other(v-if="diary.url")
            a(:href="diary.url")
               引用链接：{{diary.url}}
        div.wuji-other(v-if="diary.MediaChildren &&diary.MediaChildren.length > 0")
            el-row(:gutter="20")
    ...
    export default{
        ...
        methods:{
            ...
            getDiaryDetail(id){
                ...
                let _self = this, params = { keyValue: id ,name:"testname"};
                //params => 参数
            ...
            getMediaArr(flag){
                let arr = [];
                const len = this.diary.MediaChildren?this.diary.MediaChildren.length: 0;
